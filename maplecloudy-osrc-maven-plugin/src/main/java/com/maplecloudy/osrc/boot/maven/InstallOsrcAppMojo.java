...
import com.maplecloudy.osrc.boot.HttpUtils;
import com.maplecloudy.osrc.model.app.App;
import com.maplecloudy.osrc.model.app.Config;
...
public class InstallOsrcAppMojo extends AbstractMojo {
   ...
   * @since 2.4
   */
  @Parameter(property = "install.osrc.skip", defaultValue = "false")
  private boolean skip;

  @Override
  public void execute() throws MojoExecutionException, MojoFailureException {
    if (skip) {
      getLog().info("Skipping install osrc app");
    } else {
      installProject();
    }
  }
  ...
  private void installProject()
    ...
    try {
      ...
      getLog().info("install osrc Artifact:" + artifact);
      String packaging = project.getPackaging();
      File pomFile = project.getFile();

      List<Artifact> attachedArtifacts = project.getAttachedArtifacts();

      getLog().info("install osrc attachedArtifacts:" + attachedArtifacts);
      boolean isPomArtifact = "pom".equals(packaging);

      ProjectArtifactMetadata metadata;

      getLog().info("install osrc isPomArtifact:" + isPomArtifact);
      if (isPomArtifact)// app install ignore pom Artifact
      {
        getLog().info("install osrc skip Pom Artifact!");
        return;
      }
      ...
      try {
        ...
        getLog().info("install osrc file:" + target);
        ...
        if (indexEntry == null) {
          getLog().error("install osrc skip! Package not have [index.yml]!");
          return;
        }
        ...
        if (respStatus == 200) {
          getLog().info("install osrc file:" + target + " success!");
          getLog().info("App info: " + m.getResponseBodyAsString());
        } else {
          getLog().error(
              "install osrc file:" + target + " failed with code: " + respStatus
  ...
  private boolean checkOsrcFileValid(File osrcFile, Header header, Scanner sc)
    ...
    if (!osrcFile.exists()) {
      ...
      Scope scope = initScope(sc, osrcAppSite, header);
      config.setScope(scope);
  ...
  private boolean checkScopeValid(String osrcAppSite, Scope scope,
    ...
    String accessStr = HttpUtils.doGet(osrcAppSite + "/api/users/deploy/access",
        maps, header);
  ...
  private boolean checkLoginTokenValid(String osrcAppSite, Header header)
      throws JsonProcessingException {
    String userInfoStr = HttpUtils.doGet(osrcAppSite + "/api/users", null,
        header);
  ...
  private boolean initLoginInfoSuccess(Config config, String osrcAppSite,
      Header header, Scanner sc) throws JsonProcessingException {
    ...
    while (true) {
      ...
      if ("y".equalsIgnoreCase(yn) || "yes".equalsIgnoreCase(yn)) {
        ...
        String userTokenStr = HttpUtils.doPost(
            osrcAppSite + "/api/users/signin", om.writeValueAsString(mapPara),
  ...
  private FilePart executeAppCheck(JarFile targerJar, File target,
      HttpClient hc, String osrcAppSite, Header header, JarEntry indexEntry)
      throws IOException {
    ...
    PostMethod check = new PostMethod(
        osrcAppSite + "/api/apps/check?scopeId" + "=" + config.getScope()
    ...
    if (code == 200) {
      ...
      getLog().error(
          "install osrc file:" + target + " failed with code: " + code
  ...
  private Scope initScope(Scanner sc, String osrcAppSite, Header header)
      throws JsonProcessingException {
    ...
    while (true) {
      ...
      if (type == 1) {
        ...
        String countStr = HttpUtils.doGet(
            osrcAppSite + "/api/organizations/count", null, header);
        if (ObjectUtils.isEmpty(countStr) || Integer.valueOf(countStr) == 0) {
          ...
          while (true) {
            ...
            String memberRoleStr = HttpUtils.doGet(
                osrcAppSite + "/api/organizations/role", nameMap, header);
  ...
  private String initProject(Scanner sc, String osrcAppSite, Header header)
      throws JsonProcessingException {
    ...
    while (true) {
      ...
      if ("y".equalsIgnoreCase(yn)) {
        ...
        String prjStr = HttpUtils.doGet(osrcAppSite + "/api/projects/check",
            nameMap, header);
        if (StringUtils.isNotBlank(prjStr)) {
          ...
          HttpUtils.doPost(osrcAppSite + "/api/projects/app-bundle", map,
              header);
          break;
        } else {
        ...
        while (true) {
          ...
          getLog().info("(project name can only consist of up to 100 "
              + "lowercase characters.Hyphens can be used in "
        ...
        nameMap.put("scopeId", Integer.valueOf(config.getScope().getId()));
        nameMap.put("type", config.getScope().getType());
        String prjStr = HttpUtils.doPost(osrcAppSite + "/api/projects/app-add",
            nameMap, header);
        if (StringUtils.isNotBlank(prjStr)) {
          Map<String,Object> prjMap = om.readValue(prjStr, Map.class);
          projectMap.put("projectId", prjMap.get("id"));
