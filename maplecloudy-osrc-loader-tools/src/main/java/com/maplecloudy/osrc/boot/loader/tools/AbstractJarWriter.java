...
public abstract class AbstractJarWriter implements LoaderClassesWriter {
  ...
  final void writeEntries(JarFile jarFile, EntryTransformer entryTransformer,
    ...
    while (entries.hasMoreElements()) {
      ...
      if (library == null || library.isIncluded()) {
        writeEntry(jarFile, entryTransformer, unpackHandler,
            new JarArchiveEntry(entry), library);
      }
    }
  }
  ...
  public void writeLoaderClasses(String loaderJarResourceName)
      throws IOException {
    URL loaderJar = getClass().getClassLoader()
        .getResource(loaderJarResourceName);
  ...
  private void writeEntry(JarArchiveEntry entry, Library library,
    ...
    if (this.writtenEntries.add(name)) {
      ...
      entryWriter = addUnpackCommentIfNecessary(entry, entryWriter,
          unpackHandler);
